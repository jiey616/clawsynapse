...
`node` 表示网络中的一个实例身份。每个运行中的 `clawsynapsed` 都持有本地 Ed25519 密钥，并从该公钥确定性派生：

- `did`：规范身份，当前格式为 `did:key`
- `nodeId`：运行时节点标识，用于 NATS subject、日志和 API 返回

`nodeId` 不再由用户手工配置，而是由 `did` 自动派生，当前格式为 `n1-<lowercase-hex>`。
...
这些定义的规范真源统一收敛到 `docs/protocol.md`；本节只描述架构分层，不维护字段级协议细节。
...
建议把 peer 目录视为“本地运行时数据模型”，而不是项目级协议真源；涉及字段命名、可见性与消息格式时，应以 `docs/protocol.md` 为准。
