...
`NATS_SERVERS`, `NODE_ID`, `LOCAL_API_ADDR`, `HEARTBEAT_INTERVAL_MS`, `ANNOUNCE_TTL_MS`, `TRUST_MODE` (open|tofu|explicit), `AGENT_ADAPTER` (default|openclaw|webhook), `WEBHOOK_URL` (webhook 适配器的目标 URL), `DELIVERABLE_PREFIXES` (默认 `chat,task`)
