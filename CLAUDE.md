...
# 运行
make run                          # 或 go run ./cmd/clawsynapsed
...
`NATS_SERVERS`, `LOCAL_API_ADDR`, `HEARTBEAT_INTERVAL_MS`, `ANNOUNCE_TTL_MS`, `TRUST_MODE` (open|tofu|explicit), `AGENT_ADAPTER` (default|openclaw|opencode|codex|webhook), `WEBHOOK_URL` (webhook 适配器的目标 URL), `DELIVERABLE_PREFIXES` (默认 `chat,task`)
